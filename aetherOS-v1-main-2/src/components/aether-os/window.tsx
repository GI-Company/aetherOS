...
import type { WindowInstance } from "@/app/apps/desktop";
...
export default function Window({
  ...
  const appTutorial = TUTORIALS[app.manifest.id];
  const isAppTutorialHidden = userPreferences?.tutorials?.hiddenAppTutorials?.includes(app.manifest.id);
  ...
  }, [app.manifest.id, appTutorial, isAppTutorialHidden]);


  const getDockPosition = () => {
    if (dockRef.current) {
      const dockIcon = dockRef.current.querySelector(`[data-app-id="${app.manifest.id}"]`);
      if (dockIcon) {
  ...
  }, [isMinimized, position, size, api, id, app.manifest.id, size.width, size.height]);
  ...
  const bindResize = useDrag(
    ({ down, movement: [mx, my], last, memo }) => {
      ...
      const newWidth = Math.max(app.manifest.ui_hints.defaultSize.width, memo[0] + mx);
      const newHeight = Math.max(app.manifest.ui_hints.defaultSize.height, memo[1] + my);
  ...
  const handleFinishAppTutorial = (dontShowAgain: boolean) => {
    ...
    if (dontShowAgain) {
        onHideTutorial(app.manifest.id);
    }
  };
      ...
      <AetherAppProvider appId={app.manifest.id}>
        ...
        >
            ...
            <div className="flex items-center gap-2" style={{ pointerEvents: 'none' }}>
                ...
                <span className="text-sm font-medium select-none">{app.manifest.name}{isDirty ? '*' : ''}</span>
            </div>
