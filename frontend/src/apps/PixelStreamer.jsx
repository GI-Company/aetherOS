...
// No longer need direct Firebase access from this component
...
export default function PixelStreamerApp() {
  ...
  const handleSaveImage = async () => {
    if (!generatedImage || !aether) {
      toast({ title: "No image to save or aether client not available.", variant: "destructive" });
      return;
    }
    ...
    // The user's root is now managed by the backend, let's assume /home/user for now
    const filePath = `/home/user/${fileName}`;
    
    aether.publish('vfs:write', { path: filePath, content: generatedImage });
  ...
  return (
    <div className="h-full w-full flex flex-col p-4 gap-4 bg-gray-800 text-white">
       <CardHeader>
      ...
      <Card className="flex-grow relative bg-gray-900/50 overflow-hidden border-gray-700">
        <CardContent className="p-2 h-full">
            <div className="relative w-full h-full flex items-center justify-center rounded-md overflow-hidden bg-gray-800/50">
             {isLoading === 'generate' ? (
